...
class TestSchedulesScalesPar extends AnyFunSuite{
  ...
  test("Test schedulesScales 1") {
    val its1 = flightSchedulesPar.schedulesScales(vuelosCurso, aeropuertosCurso)("MID", "SVCS")
    assert(its1 === List(
  ...
  test("Test schedulesScales 2") {
    val its2 = flightSchedulesPar.schedulesScales(vuelosCurso, aeropuertosCurso)("CLO", "SVCS")
    assert(its2 === List())
  }

  test("Test schedulesScales 3") {
    val its3 = flightSchedulesPar.schedulesScales(vuelosCurso, aeropuertosCurso)("CLO", "SVO")
    assert(its3 === List(
  ...
  test("Test schedulesScales 4") {
    val its4 = flightSchedulesPar.schedulesScales(vuelosCurso, aeropuertosCurso)("CLO", "MEX")
    assert(its4 === List(
      Itinerario(List(Vuelo("AVA", 9432, "CLO", 7, 0, "BOG", 8, 0, 0),
  ...
  test("Test schedulesScales 5") {
    val its5 = flightSchedulesPar.schedulesScales(vuelosCurso, aeropuertosCurso)("CTG", "PTY")
    assert(its5 === List(
