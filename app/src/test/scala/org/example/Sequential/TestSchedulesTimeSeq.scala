...
class TestSchedulesTimeSeq extends AnyFunSuite {
  val itoObj = new FlightSchedules()

  test("test1") {
    val its1 = itoObj.schedulesTime(vuelosCurso, aeropuertosCurso)("MID", "SVCS")
    assert(its1 === List(Itinerario(List(Vuelo("AIRVZLA", 601, "MID", 5, 0, "SVCS", 6, 0, 0)), 60, 0, 60)))
  }

  test("test2") {
    val its2 = itoObj.schedulesTime(vuelosCurso, aeropuertosCurso)("CLO", "SVCS")
    assert(its2 === List())
  }

  test("test3") {
    val its3 = itoObj.schedulesTime(vuelosCurso, aeropuertosCurso)("CLO", "SVO")
    assert(its3== List(Itinerario(List(Vuelo(
      "AVA", 9432, "CLO", 7, 0, "SVO", 2, 20, 4)), 680, 4, 680),
  ...
  test("test4") {
    val its4 = itoObj.schedulesTime(vuelosCurso, aeropuertosCurso)("CLO", "MEX")
    assert(its4 === List(
      Itinerario(List(Vuelo("AVA", 9432, "CLO", 7, 0, "BOG", 8, 0, 0),
  ...
  test("test5") {
    val its5 = itoObj.schedulesTime(vuelosCurso, aeropuertosCurso)("CTG", "PTY")
    assert(its5 === List(
      Itinerario(List(Vuelo("COPA", 1234, "CTG", 10, 0, "PTY", 11, 30, 0)), 90, 0, 90),
      Itinerario(List(Vuelo("AVA", 4321, "CTG", 9, 30, "SMR", 10, 0, 0),
